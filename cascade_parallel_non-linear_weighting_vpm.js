...
/** Changelog
	* version 0.1.5 dated October 14th, 2024 by gitisgreat2023
	* - added function DistributionInverters. This is an explicit version only suited for a 1500W with 2250W combination.
	*
...
// receive new controller Power values 
...
// calculate contribution of inverters
function DistributionInverters(pwr, index){
	var p1=135, p2=250, p3=62.5, p4=187.5;
	switch(index){
		case 0:
		if(pwr <0){
			return pwr;
		...
		if(pwr>175 & pwr<=p2){
			return (p3-p1)/(p2-p1)*(pwr-p1)+p1;
		}
		if(pwr>p2 & pwr<=3000){
			return pwr*0.25;
		}
		if(pwr>3000){
			return (1500-750)/(3750-3000)*(pwr-3000)+750;
		}
		break;		
		case 1:
		if(pwr<=p1){
...
function calculateVirtualPowerReadings( index){
	switch (algo_mode.toLowerCase()){
		...
		//print("index: "+index+" powersplit: "+powersplit+" contribution: "+DistributionInverters(powersplit, index)+ " prev: "+previousPower[index])
		var limitedPowerMeter = 0; 
		if (huaweiPower[0] == 0){
			limitedPowerMeter = DistributionInverters(powersplit, index) - previousPower[index];
		}
		if(index == 1 && huaweiPower[0] != 0){
			limitedPowerMeter = -2250;
		}
		if(index == 0 && huaweiPower[0] != 0){
			limitedPowerMeter = powersplit;
		}
		break;
		default:
